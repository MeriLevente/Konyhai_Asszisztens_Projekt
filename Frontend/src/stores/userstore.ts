...
export const useUserStore = defineStore('userStore', {
    ...
    actions: {
        login(data: IUser) {
            let validation: IFormResponse = UserValidation.LoginAllFilled(data.email, data.password)
        ...
        register(data: IUser){
            let validation: IFormResponse = UserValidation.RegisterAllFilled(data.name!, data.email, data.password, this.status.confirm_password)
            if(!validation.isError){
                validation = UserValidation.PasswordInCorrectForm(data.password)
                if(!validation.isError){
                    validation = UserValidation.PasswordsMatch(data.password, this.status.confirm_password)
                    if(!validation.isError){
